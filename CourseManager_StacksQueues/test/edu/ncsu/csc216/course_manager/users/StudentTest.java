...
public class StudentTest {
	...
	/**
	 * Tests that a Student is constructed correctly. First tests the constructor
	 * that does not pass in the max credit value, then tests the constructor
	 * that passes in the max credit value, then lastly tests the constructor
	 * that does not pass in the max credit value, the password value, or the
	 * email value.
	 */
	@Test
	public void testStudent() {
		...
		assertEquals(id + "@ncsu.edu", s2.getEmail());
		assertEquals(id, s2.getPassword());
		assertEquals(Student.MAX_CREDITS, s2.getMaxCredits());
		assertEquals(0, s1.getCourses().length);
		assertEquals(0, s1.getCurrentCredits());
	}
